...
import { BlogData } from "../../store/data/data.types";
import BlogItem from "../../components/blog/blog-item.component";
import innerBg from "../../assets/img/inner-bg.png";
import innerBg2 from "../../assets/img/inner-bg-2.png";
import { useSelector } from "react-redux";
import { selectTheme } from "../../store/theme/theme.selector";

const Blog = () => {
  const [blogData, setBlog] = useState<BlogData>();
  const [loading, setLoading] = useState(true);
  const { slug } = useParams();
  const theme = useSelector(selectTheme),
    headerBg = theme === "dark" ? innerBg : innerBg2;
  const getData = async (uri = `${hostURL}/api/get/blog`) => {
      setLoading(true);
      try {
  ...
  if (blogData == null) return <Preloader />;
  if (loading) return <Preloader />;
  const { data, meta } = blogData,
    projects = data;
  return (
    <div
      ...
      data-name="portfolio">
      <div
        className="bg-cover bg-bottom bg-no-repeat pt-[82px] lg:pt-[106px]"
        style={{
          ...
          <div className="container">
            <div className="items-center justify-between py-10 md:flex md:h-[400px] md:py-0">
              <div className="heading relative mb-0 text-center ltr:md:text-left rtl:md:text-right">
                <h6>Blog</h6>
                <h4 className="!text-white">Get tips and news</h4>
              </div>
              <div
                className="relative mt-6 md:mt-0 aos-init aos-animate"
                data-aos="fade-left"
